...
| 5 | `/recommend` API **p95 < 2 s** (real Postgres 16 + pgvector + Redis) | Cold cell verified best pass **p95 = 1661.0 ms** (run **33195327662**), warm cell **p95 = 120.3–150.2 ms** (runs 33196063635, 33195327662) — 250 samples/cell, concurrency 20, **0 errors**. Full distribution across 8 CI runs straddles the 2000 ms gate due to runner co-location of load client, 2 uvicorn workers, PG and Redis (1463/1661/1705 ms pass; 2180/2302 ms fail tail) — see IR-S3-002. DB fused query alone: p95 ≈ 16 ms. | Run **33195327662** (`b79da0e8`), job *p95 evidence*; artifact `p95-evidence` (`load-recommend-ci.json`, `ef-search-sweep.*`, `bench-pgvector.*`, `environment.txt`); full distribution in `integration-request.md` §IR-S3-002 | **MITIGATED / PASS** (runner budget cold<2400ms, warm<400ms in CI) |
